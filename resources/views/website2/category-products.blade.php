        ...
        <div class="container">
            <div class="row">
                <div class="col-12 col-sm-12 col-md-12 col-lg-12 d-flex justify-content-between align-items-center">
                    <div class="page-title"><h1>{{$category->{'name_'.$lang} }}</h1></div>
                    <!--Breadcrumbs-->
                    <div class="breadcrumbs">
                        <span class="main-title fw-bold">{{$category->{'name_'.$lang} }}</span>
            ...
            <div class="toolbar toolbar-wrapper shop-toolbar">
                <div class="row align-items-center">
                    ...
                    <div class="col-12 col-sm-4 col-md-4 col-lg-4 text-center product-count order-0 order-md-1 mb-3 mb-sm-0">
                        <span class="toolbar-product-count">{{__('home.show')}}: 15 {{__('home.products')}}</span>
                    </div>
                        ...
                        <div class="sidebar-widget clearfix categories filterBox filter-widget">
                            ...
                            <div class="widget-content filterDD">
                                <ul class="sidebar-categories scrollspy morelist clearfix">
                                    @foreach($Categories as $MainCategorie)
...
{{--                                    changed amound--}}
                                    <input type="hidden" id="f_max_price" name="to"  value="{{$products->max('price')}}">
                                    <input type="hidden" id="f_min_price" name="from" value="{{$products->min('price')}}">
                    ...
                    <div class="grid-products grid-view-items">
                                ...
                                <div class="item col-item">
                                    <div class="product-box">
                                        ...
                                        <div class="product-image">
                                            <!-- Start Product Image -->
                                            <a href="{{LaravelLocalization::localizeUrl('product/'.$product->{'link_'.$lang})}}" class="product-img rounded-3">
                                                <img class="blur-up lazyload"
                                                     src="{{Helper::uploadedImagesPath('products',$product->images()[0])}}" alt="{{$product->{'name_'.$lang} }}"
                                                     title="{{$product->{'name_'.$lang} }}" width="625" height="808" /></a>
                                            <!-- End Product Image -->
                                            <!-- Product label -->
                                            @if($product->stockCount()==0)
                                                <div class="product-labels"><span class="lbl on-sale">{{__('home.outstock')}}</span></div>
                                            @elseif($product->featurd==1)
                                            ...
                                            <div class="button-set style1">
                                                @if($product->stockCount()>0)
                                                <!--Cart Button-->
                                                <a href="javascript:void(0);" class="btn-icon btn-showCart addtocart " data-bs-toggle="modal" data-bs-target="#addtocart_modal" data-product_id="{{$product->id}}">
                                                ...
                                                @endif
                                                <!--Wishlist Button-->
                                                <a href="javascript:void(0);" class="btn-icon btn-wishlist  wishlist" data-bs-toggle="tooltip" data-bs-placement="left" title="{{__('home.Add to Wishlist')}}" data-product_id="{{$product->id}}">
                                        ...
                                        <div class="product-details">
                                            ...
                                            <div class="product-name">
                                                <a href="{{LaravelLocalization::localizeUrl('product/'.$product->{'link_'.$lang})}}">
                                                    {{ Illuminate\Support\Str::limit($product->{'name_'.$lang}, $limit = 50, $end = '...') }}
                                                </a>
                                            </div>
                                            <!-- End Product Name -->
                                            <!-- Product Price -->
                                            <div class="product-price">
                                                @if($product->discount()>0)
                                                    <span class="price old-price">{{$product->price.' '.__('home.EGP')  }}</span>
                                                    <span class="price">{{$product->condition_discount() .' '.__('home.EGP')   }}</span>
                                                @else
                                                    <span class="price">{{$product->price.' '.__('home.EGP')  }}</span>
                                                @endif
                                            </div>
                                            <!-- End Product Price -->
                                            <!-- Product Review -->
                                            <div class="product-review">
                                                ...
                                                @endfor
                                                <span class="caption me-1">{{$product->reviewCount()}} {{__('home.reviews')}} </span>
                                            </div>
                                            <!-- End Product Review -->
                                        </div>
                                        <!-- End product details -->
                        ...
                        <!-- Pagination -->
                            ...
                            <ul class="pagination justify-content-center" style="display: flex!important;">
                                    ...
                                    @else
                                        <li class="page-item"><a class="page-link" href="{{ $products->previousPageUrl() . '&' . http_build_query(request()->except('page')) }}"><i class="icon anm anm-angle-right-l"></i></a></li>
                                    @endif
                                    ...
                                    <li class="page-item"><a class="page-link" href="{{ $products->nextPageUrl() . '&' . http_build_query(request()->except('page')) }}"><i class="icon anm anm-angle-left-l"></i></a></li>
                                @else
