                ...
                <div class="row">
                    <div class="col-md-6 col-lg-6">
                        <div class="card py-4">
                            ...
                            <div class="card-footer pt-4 bg-transparent">
                                    ...
                                    <table class="bg-white table table-bordered table-hover text-center">
                                        ...
                                        <tbody>
                                            @foreach($cartProducts as $product)
                                                <tr>
                                                	<td>
                                                	    <a href="{{($lang == 'en')?url('product/'.$product->group->product->link_en):url('product/'.$product->group->product->link_ar)}}" class="thumb">
                                                	        @if($product->group->image->image)
                                                                <img width="100px" class="checkout-image" src="{{url('uploads/products/resize200/'.$product->group->image->image)}}">
                                                            @else
                                                    ...
                                                    <td>
                                                        @if($product->group->product->discount)
                                                            <span>{{$product->group->price_details->new_price}} {{trans('home.SAR')}}</span>
                                                            <del class="product-old-price">{{$product->group->price_details->old_price}} {{trans('home.SAR')}}</del>
                                                        @else
                                                            <span>{{$product->group->price_details->new_price}} {{trans('home.SAR')}}</span>
                                                        @endif
                                                    </td>
                                                    <td>{{$product->quantity}}</td>
                                                    <td>{{$product->total_product }} {{trans('home.SAR')}}</td>
                                                </tr>
                                            @endforeach
        ...
        'ecommerce': {
            'checkout': {
                ...
                'items': [
                    ...
                    {
                        'id': '{{ $cartProduct->group->product->id }}', // Product ID
                        'name': '{{ $cartProduct->group->product->name }}', // Product name
                        'category': '{{ $cartProduct->group->product->category->name }}', // Category
                        'price': '{{ $cartProduct->group->price }}', // Price
