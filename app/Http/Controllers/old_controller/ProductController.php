...
class ProductController extends Controller
{
    ...
    {
        $categories=Category::where('status',1)->whereDoesntHave('subCategories')->get();
        ...
        $max = count($chars)-1;
        for($i=0;$i<6;$i++){
            $code .= (!($i % 6) && $i ? '-' : '').$chars[rand(0, $max)];
        }
        return view('admin.products.addProduct',compact('categories','code','colors','brands'));
    }
